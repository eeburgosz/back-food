	...
	if (data.length === 0) {
		...
		try {
			...
			for (const recipeInstance of recipesInstances) {
				const diets = recipesData.find(
					(recipe) => recipe.id === recipeInstance.id
				).diets;
				await recipeInstance.setTypes(
					diets.map((dietName) => dietsMap.get(dietName).id)
				);
			}
			console.log(
				"Recetas y tipos de dieta creados y asociados correctamente."
			);
			return await Recipes.findAll();
		} catch (error) {
			console.error(
				"Error al crear y asociar recetas y tipos de dieta:",
				error
			);
		}
	}
